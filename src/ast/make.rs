...
impl<'src> expr::Primitive<'src> {
  ...
  pub fn new_str<T: Default>(span: impl Into<Span>, value: impl Into<Str<'src>>) -> Expr<'src, T> {
    Expr {
      span: span.into(),
      ty: Default::default(),
...
impl<'src, T> Block<'src, T> {
  pub fn new(span: Span, body: Vec<Stmt<'src, T>>, tail: Option<Expr<'src, T>>) -> Block<'src, T> {
